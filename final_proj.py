...
from final_proj_utils import get_leaves_to_root
...
def compute_empirical_distribution(values):
    ...
    distribution = {x: values.count(x)/No_values for x in unique_values }
...
def compute_empirical_mutual_info_nats(var1_values, var2_values):
    ...
    var1_var2_joint = list(zip(var1_values,var2_values))
    joint_distribution = compute_empirical_distribution(var1_var2_joint)
    

    empirical_mutual_info_nats = 0.0
    
    for obs in var1_var2_joint:
        empirical_mutual_info_nats += joint_distribution[obs]* \
            np.log(joint_distribution[obs]/(var1_distribution[obs[0]]*var2_distribution[obs[1]]))
...
def chow_liu(observations):
    ...
    #Get mututal information of all possible pairs of nodes
    pairs = list(combinations(range(num_vars),2))
    mut_info = [(pair,compute_empirical_mutual_info_nats(observations[:,pair[0]],\
                                                        observations[:,pair[1]])) for pair in pairs]

    #Sort edges in descending order of muttual information
    sorted_edges = [ x for x,_ in sorted(mut_info, key = lambda pair:pair[1])[::-1]]
    
    ###Build network using sorted edges
...
def compute_empirical_conditional_distribution(var1_values, var2_values):
    ...
    var1_unique_values = list(set(var1_values))
   # var2_unique_values = list(set(var2_values))
    
    conditional_distributions= {}
    
    for var2 in var2_values:
        var1_values_for_var2 = list(var1_values[var2_values==var2])
        No_values = len(var1_values_for_var2)
        conditional_distributions[var2] = {value:var1_values_for_var2.count(value)/No_values for \
                                             value in var1_unique_values  }
...
def learn_tree_parameters(observations, tree, root_node=0):
    ...
    #Get all possible edge potentials by traversing the edges
    ...
    while len(fringe)>0:
        node = fringe.pop(0) #removes the 0th element of fringe and return it
        visited[node] = True #marked 'node' as visited
        for neighbor in edges[node]:
            if not visited[neighbor]:
                edge_potentials[(node,neighbor)] = \
                compute_empirical_conditional_distribution(observations[:,node],observations[:,neighbor])
...
def sum_product(nodes, edges, node_potentials, edge_potentials):
    ...
    #Convert node and edge potentials into arrays/matrices
    node_potential_array = { x: np.array(list(node_potentials[x].values())) for x in node_potentials.keys()}
    #edge_potential_list = [edge_potentials[x] for x in edge_potentials.keys()]
    edge_potential_matrix = {}
    for x in edge_potentials.keys():
        edge_potential = edge_potentials[x]
        edge_potential_matrix[x] = np.array([list(edge_potential[x].values()) for \
                             x in edge_potential.keys()])
    ...
    ###Get leaves to root for forward messages####
    for root_node in node_potentials.keys():
        ...
        #Multiply with node potential
        marginal = marginal * node_potential_array[root_node]
...
def compute_marginals_given_observations(nodes, edges, node_potentials,
    ...
    """
    new_node_potentials = {}
    ##Get values node can take
    node_potential_values = list(list(node_potentials.values())[0].keys())
    default_potentials1 = {x:0 for x in node_potential_values}
    default_potentials2 = {x:1 for x in node_potential_values}
    ####All node potentials exept root node can be uniform i.e. = 1
    ...
    # YOUR CODE HERE
    #
    ##############update node potentials based on observations###########
    for node in nodes:
        if node in node_potentials.keys():
            new_node_potentials[node] = node_potentials[node]
        elif node in observations.keys():
            new_node_potentials[node] = default_potentials1
            observed = observations[node]
            new_node_potentials[node][observed] = 1
        else:
            new_node_potentials[node] = default_potentials2
