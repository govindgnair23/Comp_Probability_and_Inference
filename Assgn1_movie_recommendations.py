...
def compute_posterior(prior, likelihood, y):
    ...
    log_likelihood = np.log(likelihood[y,])
    num = log_prior + np.sum(log_likelihood,axis=0)
    denom = scipy.special.logsumexp(num)
    log_answer = num - denom
    #
...
def infer_true_movie_ratings(num_observations=-1):
    ...
    for i in range(num_movies):
        y = movie_data_helper.get_ratings(movie_id_list[i])[:num_observations]
        posterior_movie = compute_posterior(prior, likelihood, y)
        posteriors[i,:] = posterior_movie
        MAP_ratings[i] = np.argmax(posterior_movie)
...
def compute_entropy(distribution):
    ...
    for i in range(N):
        if(distribution[i]==0):
            ...
            entropy = entropy + distribution[i]*np.log2(1/distribution[i])
...
def compute_true_movie_rating_posterior_entropies(num_observations):
    ...
    (posteriors,_) = infer_true_movie_ratings(num_observations=-1)
...
############Test cases###################
 ...
 compute_posterior(np.array([ 0.5, 0.5]), np.array([[ 0.45, 0.6 ], [ 0.55, 0.4 ]]),
